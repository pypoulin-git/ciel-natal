          ...
          <>
            <section className="relative overflow-hidden min-h-[68vh] flex flex-col items-center justify-center px-4 text-center pt-10">
              ...
              <div className="relative animate-fade-in-up">
                ...
                <h1 className="font-cinzel text-3xl sm:text-4xl md:text-6xl font-bold bg-gradient-to-r from-[var(--color-text-primary)] to-[var(--color-accent-lavender)] bg-clip-text text-transparent leading-tight">
                  {locale === "fr" ? "Ta carte natale" : "Your natal chart"}
                </h1>
                <TitleSparkles
                  id="hero-sparkles"
            ...
            <section className="max-w-4xl mx-auto px-4 py-12">
              <div className="text-center mb-8">
                ...
                <h2 className="font-cinzel text-2xl sm:text-3xl text-[var(--color-text-primary)] mb-3">
                  {locale === "fr" ? "Qu'est-ce qu'une carte natale ?" : "What is a natal chart?"}
                </h2>
                <p className="text-sm text-[var(--color-text-secondary)] max-w-2xl mx-auto leading-relaxed">
                  {locale === "fr"
                    ? "C'est la photographie du ciel à l'instant précis de ta naissance — la position du Soleil, de la Lune et des planètes vue depuis ton lieu de naissance. Cette carte du ciel se lit comme un portrait psychologique : elle n'annonce rien, elle éclaire ta manière d'être."
                    : "It's a photograph of the sky at the exact moment of your birth — the position of the Sun, Moon and planets seen from your birthplace. This chart reads like a psychological portrait: it predicts nothing, it illuminates the way you are."}
                </p>
              </div>
              <div className="grid sm:grid-cols-3 gap-4">
                {[
                  {
                    icon: "☉",
                    t: locale === "fr" ? "Soleil, Lune, Ascendant" : "Sun, Moon, Ascendant",
                    d: locale === "fr" ? "Tes trois piliers : ton identité, ton monde émotionnel, le masque que tu portes." : "Your three pillars: identity, emotional world, and the mask you wear.",
                  },
                  {
                    icon: "♃",
                    t: locale === "fr" ? "Planètes & maisons" : "Planets & houses",
                    d: locale === "fr" ? "Chaque planète colore une facette de toi, dans un domaine concret de ta vie." : "Each planet colours a facet of you, in a concrete area of your life.",
                  },
                  {
                    icon: "△",
                    t: locale === "fr" ? "Aspects & éléments" : "Aspects & elements",
                    d: locale === "fr" ? "Les dialogues entre tes planètes et l'équilibre Feu · Terre · Air · Eau." : "The dialogues between your planets and the Fire · Earth · Air · Water balance.",
                  },
                ].map((c, i) => (
                  <div key={i} className="glass p-5 text-center">
                    <div className="text-2xl text-[var(--color-accent-gold)] mb-2">{c.icon}</div>
                    <h3 className="font-cinzel text-base text-[var(--color-text-primary)] mb-1.5">{c.t}</h3>
                    <p className="text-xs text-[var(--color-text-secondary)] leading-relaxed">{c.d}</p>
                  </div>
                ))}
              </div>
            </section>

            {/* ═══ BLOG — notions de carte natale ═══ */}
            <LatestArticles
              title={locale === "fr" ? "Pour lire ta carte" : "To read your chart"}
              slugs={[
            ...
            <section className="max-w-3xl mx-auto px-4 py-12">
              ...
              <div className="space-y-3">
                {[
                  {
                    fr: { q: "Et si je ne connais pas mon heure de naissance ?", a: "Tu peux quand même calculer ta carte sans l'heure — on te donne ce qui est sûr (Soleil, Lune approximative, planètes). L'Ascendant et les maisons nécessitent l'heure exacte, souvent inscrite sur ton acte de naissance ou disponible à l'état civil de ta ville." },
                    en: { q: "What if I don't know my birth time?", a: "You can still calculate without a time — we give you what's certain (Sun, approximate Moon, planets). The Ascendant and houses need the exact time, often on your birth certificate or available from your city's civil registry." },
                  },
                  {
                    fr: { q: "Pourquoi le lieu de naissance compte-t-il ?", a: "L'Ascendant et les maisons dépendent de l'horizon local : deux personnes nées à la même heure mais dans deux villes différentes n'ont pas le même ciel qui se lève. Le lieu affine tout le calcul." },
                    en: { q: "Why does the birth place matter?", a: "The Ascendant and houses depend on the local horizon: two people born at the same time in different cities don't have the same sky rising. The place refines the whole calculation." },
                  },
                  {
                    fr: { q: "Les calculs sont-ils fiables ?", a: "Oui — positions planétaires par éphéméride géocentrique (précision ~1°, rétrogrades et stations inclus), validées par une suite de tests automatisés contre des positions connues. Rien d'un horoscope de magazine." },
                    en: { q: "Are the calculations reliable?", a: "Yes — planetary positions from a geocentric ephemeris (≈1° accuracy, retrogrades and stations included), validated by an automated test suite against known positions. Nothing like a magazine horoscope." },
                  },
                  {
                    fr: { q: "Puis-je retrouver et re-télécharger mes cartes ?", a: "Oui, avec un compte : tes cartes sont sauvegardées dans « Mes cartes natales », rouvrables et re-générables en PDF à tout moment." },
                    en: { q: "Can I find and re-download my charts?", a: "Yes, with an account: your charts are saved under \"My natal charts\", re-openable and re-generatable as PDF anytime." },
                  },
                ].map((item, i) => (
