...
const RELATION_FRAME_FR: Record<RelationType, string> = {
  ...
  indetermine:
    "Contexte relationnel : non précisé. Lis la synastrie de manière neutre, en couvrant les dynamiques qui pourraient se jouer en amour ou amitié selon ce qui ressort des cartes.",
};
...
const RELATION_FRAME_EN: Record<RelationType, string> = {
  ...
  indetermine:
    "Relationship type: unspecified. Read the synastry neutrally, covering dynamics that could play out across love or friendship depending on what surfaces from the charts.",
};
...
export async function POST(req: NextRequest) {
  try {
    ...
    const safeRelation: RelationType =
      relationType && ["amour", "amitie", "professionnel", "indetermine"].includes(relationType)
        ? relationType
        : "indetermine";
...
Format STRICT pour chaque titre de section : exactement deux étoiles avant et après, rien d'autre. Exemple correct : **Ce qui attire**. NE PAS utiliser : ### **Ce qui attire**, *** Ce qui attire ***, ## Ce qui attire, ni aucune variante avec dièses ou astérisques supplémentaires.
