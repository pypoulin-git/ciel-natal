...
export const config = {
  matcher: [
     ...
     * - monitoring (Sentry tunnel route — must skip Supabase session refresh
     *   to avoid breaking the error reporting transport)
     * - public folder files
     */
    "/((?!_next/static|_next/image|favicon.ico|sitemap.xml|robots.txt|monitoring|.*\\.(?:svg|png|jpg|jpeg|gif|webp)$).*)",
  ],
};
